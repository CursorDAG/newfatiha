...
import { approveTeacherSchema } from "@/lib/validation";
...
export const POST = withErrorHandling(async (
  ...
  const { adminNotes } = await validateRequest(req, approveTeacherSchema);
  ...
  // Update teacher profile with review info
  ...
  try {
    await NotificationService.create({
      ...
      message: "Ваша заявка на должность учителя одобрена. Теперь вы можете создавать курсы и уроки.",
      link: "/teacher",
    });
    ...
    logger.error({ error, teacherId: teacher.id }, "Failed to send approval notification");
  }
